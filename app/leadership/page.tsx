          ...
          {ordered.map((l) => (
            <Card key={l.id} hover className="relative">
              ...
              <div className="flex items-center gap-4">
                <div className="clip-hex relative flex h-16 w-16 items-center justify-center border border-accent-500/40 bg-accent-500/10">
                  <Crown className="h-7 w-7 text-accent-400" />
                </div>
                <div>
                  <div className="font-display text-lg font-bold text-slate-50">{l.nameAr}</div>
                  <div className="text-xs text-accent-400">{getRankTitle(l.rankId)}</div>
                  <div className="text-xs text-slate-400">{getDepartmentTitle(l.departmentId)}</div>
                </div>
              </div>
              <p className="mt-3 line-clamp-2 text-sm text-slate-300">{l.bioAr}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {l.contactEmail && (
                  <span className="flex items-center gap-1 rounded-full border border-white/10 px-2 py-1 text-[11px] text-slate-400">
